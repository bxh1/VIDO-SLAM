...
#include "3rdparty/g2o/g2o/core/optimization_algorithm_dogleg.h"
...
#include "3rdparty/g2o/g2o/types/types_dyn_slam3d.h"
#include "3rdparty/g2o/g2o/types/vertex_se3.h"
#include "3rdparty/g2o/g2o/types/vertex_pointxyz.h"
#include "3rdparty/g2o/g2o/types/edge_se3.h"
#include "3rdparty/g2o/g2o/types/edge_se3_pointxyz.h"
#include "3rdparty/g2o/g2o/types/edge_se3_prior.h"
#include "3rdparty/g2o/g2o/types/edge_xyz_prior.h"
#include "3rdparty/g2o/g2o/core/sparse_optimizer_terminate_action.h"
#include "3rdparty/g2o/g2o/solvers/linear_solver_csparse.h"
...
void Optimizer::PartialBatchOptimization(Map* pMap, const cv::Mat Calib_K, const int WINDOW_SIZE)
{
   ...
   // optimizer.setVerbose(true);
    ...
    bool check_before_opt=false, check_after_opt=false;
    if (check_before_opt)
    {
...
void Optimizer::LocalInertialBA(Map* pMap, const cv::Mat Calib_K, const int WINDOW_SIZE)
{
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale)
{
    ...
    int its = 10; // Check number of iterations
    const vector<Frame*> vpFs = pMap->GetFramesInMap();
    long unsigned int maxKFid = vpFs.back()->mnId;
    // Setup optimizer
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
    ...
    // Set Frame vertices (fixed poses and optimizable velocities)
    for(size_t i=0; i<vpFs.size(); i++)
    {
        Frame* pFi = vpFs[i];
        if(pFi->mnId>maxKFid)
            continue;
        VertexPose * VP = new VertexPose(pFi);
        VP->setId(pFi->mnId);
        VP->setFixed(true);
        optimizer.addVertex(VP);

        VertexVelocity* VV = new VertexVelocity(pFi);
        VV->setId(maxKFid+(pFi->mnId)+1);
        VV->setFixed(true);
        optimizer.addVertex(VV);

        // Biases
        VertexGyroBias* VG = new VertexGyroBias(vpFs.front());
        VG->setId(2*(maxKFid+1)+(pFi->mnId));
        VG->setFixed(true);
        optimizer.addVertex(VG);
        VertexAccBias* VA = new VertexAccBias(vpFs.front());
        VA->setId(3*(maxKFid+1)+(pFi->mnId));
        VA->setFixed(true);
    ...
    for(size_t i=0;i<vpFs.size();i++)
    {
        ...
        if(pFi->mpPrevFrame && pFi->mnId<=maxKFid)
        {
            if(pFi->mpPrevFrame->mnId>maxKFid)
                continue;
            if(!pFi->mpImuPreintegrated)
                std::cout << "Not preintegrated measurement" << std::endl;

            pFi->mpImuPreintegrated->SetNewBias(pFi->mpPrevFrame->GetImuBias());
            g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pFi->mpPrevFrame->mnId);
            g2o::HyperGraph::Vertex* VV1 = optimizer.vertex(maxKFid+(pFi->mpPrevFrame->mnId)+1);
            g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pFi->mnId);
            g2o::HyperGraph::Vertex* VV2 = optimizer.vertex(maxKFid+(pFi->mnId)+1);
            g2o::HyperGraph::Vertex* VG = optimizer.vertex(2*(maxKFid+1)+pFi->mpPrevFrame->mnId);
            g2o::HyperGraph::Vertex* VA = optimizer.vertex(3*(maxKFid+1)+pFi->mpPrevFrame->mnId);
            ...
            EdgeInertialGS* ei = new EdgeInertialGS(pFi->mpImuPreintegrated);
...
void Optimizer::InertialOptimization(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bFixedVel, float priorG, float priorA)
{
    Verbose::PrintMess("inertial optimization", Verbose::VERBOSITY_DEBUG);
    ...
    vpei.reserve(vpFs.size());
    vector<pair<Frame*,Frame*> > vppUsedF;
    vppUsedF.reserve(vpFs.size());
    ...
    for(size_t i=0;i<vpFs.size();i++)
    {
        ...
        if(pFi->mpPrevFrame && pFi->mnId<=maxKFid)
        {
            ...
            vppUsedF.push_back(make_pair(pFi->mpPrevFrame,pFi));
            optimizer.addEdge(ei);
    ...
    for(size_t i=0; i<N; i++)
    {
        ...
        VertexVelocity* VV = static_cast<VertexVelocity*>(optimizer.vertex(maxKFid+(pFi->mnId)+1));
        Eigen::Vector3d Vw = VV->estimate(); // Velocity is scaled after
        pFi->SetVelocity(Converter::toCvMat(Vw));

        if (cv::norm(pFi->GetGyroBias()-cvbg)>0.01)
        {
            pFi->SetNewBias(b);
            if (pFi->mpImuPreintegrated)
                pFi->mpImuPreintegrated->Reintegrate();
        }
        else
            pFi->SetNewBias(b);
