...
Rect selectROI(const std::string &windowName, cv::Mat& img, bool showCrosshair,
               bool fromCenter) {
  // TODO
#if CV_MAJOR_VERSION > 2
  auto rect =
      cv::selectROI(windowName, img, showCrosshair, fromCenter);
...
Rect selectROI(cv::Mat& img, bool showCrosshair, bool fromCenter) {
  // TODO
#if CV_MAJOR_VERSION > 2
  auto rect = cv::selectROI("window", img, showCrosshair, fromCenter);
  return Rect(rect.x, rect.y, rect.width, rect.height);
#else
  return Rect(-1, -1, -1, -1);
#endif
...
void selectROIs(const std::string &windowName, cv::Mat& img,
  ...
  for (auto b : boundingBoxes) {
    boxes.push_back(cv::Rect2d(b.x, b.y, b.width, b.height));
  }
  // cv::selectROI(windowName, img, boxes,
  //                fromCenter);
#endif
}
